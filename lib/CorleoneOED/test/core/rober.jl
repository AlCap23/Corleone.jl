...
prob = DAEProblem(f, du0, u0, (0.0, 10), [0.04, 3e7, 1e4]; abstol=1e-8, reltol=1e-8, differential_vars, initializealg = BrownFullBasicInit(),)

# Specify measurement
lb, ub = 1e-3, 0.95*last(prob.tspan)
...
pc1 = PiecewiseParameter(:k2, [0.0], 3e7, (3e7,3e7))
...
@test isapprox(last(sol_prob.u),  last(sol_layer.u)[1:length(oed.augmented_prob.u0)], atol=1e-6)
